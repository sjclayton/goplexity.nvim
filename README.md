...
- **Sorting**: `sort.Slice`, `sort.SliceStable`, `sort.Ints`, `sort.Strings`, `sort.Search` -> O(log n), `sort.SearchInts`, `sort.SearchFloat64s`, `sort.IsSorted`, `sort.IntsAreSorted` -> O(n), `sort.Reverse` -> O(1)
- **Containers**: `container/list` (New, PushBack, PushFront, Remove) -> O(1), `container/ring` (New, Len, Move) -> O(n), `container/heap` (Init O(n), Push, Pop) -> O(log n)
- **Strings**: `strings.Split`, `strings.Join`, `strings.Contains`, `strings.Index`, `strings.ToLower`, `strings.ToUpper`, `strings.Trim`, `strings.Replace`, `strings.Count`, `strings.HasPrefix`, `strings.HasSuffix`, `strings.Builder`, `strings.NewReader` -> O(n)
- **Bytes**: `bytes.Split`, `bytes.Join`, `bytes.Equal`, `bytes.Compare`, `bytes.NewBuffer`, `bytes.Buffer` -> O(1), `bytes.Buffer.WriteString` -> O(n)
- **Math**: `math.Abs`, `math.Max`, `math.Min`, `math.Ceil`, `math.Floor`, `math.Round`, `math.Pow`, `math.Sqrt`, `math.Log`, `math.Exp`, `math.Sin`, `math.Cos`, `math.Tan`, `math.Atan2`, `math.Pi` -> O(1), `math/bits.OnesCount`, `math/bits.LeadingZeros`, `math/bits.TrailingZeros`, `math/bits.RotateLeft` -> O(1), `math/big.NewInt`, `big.Int.Add`, `big.Int.Mul`, `big.Int.Div`, `big.Int.Sub` -> O(n)
- **I/O**: `fmt.Print`, `fmt.Println`, `fmt.Sprint`, `fmt.Sprintf`, `fmt.Fprintf`, `fmt.Errorf`, `fmt.Scan`, `fmt.Fscan`, `fmt.Sscan`, `fmt.Scanf`, `os.ReadFile`, `os.WriteFile`, `os.Read`, `os.Write`, `os.ReadDir`, `os.Open`, `os.Create`, `os.Stat`, `os.Lstat`, `bufio.NewReader`, `bufio.NewWriter`, `bufio.Scanner.Scan`, `io.ReadFull`, `io.Copy` -> O(n), `bufio.NewScanner` -> O(1)
- **Encoding**: `json.Marshal`, `json.Unmarshal`, `binary.Read`, `binary.Write`, `base64.NewDecoder`, `base64.StdEncoding`, `encoding/binary.LittleEndian` -> O(n)
...
- **Sync**: `sync.Map.Range` -> O(n), `sync.Map.Load`, `sync.Map.Store`, `sync.Map.Delete`, `sync.Mutex.Lock`, `sync.Mutex.Unlock`, `sync.Pool.Get`, `sync.Pool.Put`, `sync.WaitGroup.Add`, `sync.WaitGroup.Done`, `sync.WaitGroup.Wait`, `sync.Once.Do` -> O(1)
...
- **Time**: `time.Now`, `time.Sleep`, `time.Since`, `time.Until`, `time.Second`, `time.Hour` -> O(1)
- **Algorithms**:
