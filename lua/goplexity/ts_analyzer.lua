...
--
-- Public API (mirrors analyzer.lua):
--   M.analyze(bufnr) -> results
--
-- results = {
...
local _PLUGIN_ROOT = vim.fn.fnamemodify(
  debug.getinfo(1, 'S').source:match('@?(.+)$'), ':h:h:h'
)

local _QUERY = nil  -- cached after first parse
local function get_query()
  if _QUERY then return _QUERY end
...
local STDLIB = {
  sort = {
    Slice = 'O(n log n)', SliceStable = 'O(n log n)',
    Ints = 'O(n log n)', Strings = 'O(n log n)', Float64s = 'O(n log n)',
    Search = 'O(log n)', SearchInts = 'O(log n)',
    SearchStrings = 'O(log n)', SearchFloat64s = 'O(log n)',
    IsSorted = 'O(n)', Reverse = 'O(1)',
  },
  slices = {
    Sort = 'O(n log n)', SortFunc = 'O(n log n)', SortStableFunc = 'O(n log n)',
    BinarySearch = 'O(log n)', BinarySearchFunc = 'O(log n)',
    Equal = 'O(n)', Contains = 'O(n)', Clone = 'O(n)',
    ContainsFunc = 'O(n)', IndexFunc = 'O(n)', Index = 'O(n)',
    Delete = 'O(n)', Insert = 'O(n)',
  },
  maps    = { Keys = 'O(n)', Values = 'O(n)', Equal = 'O(n)', Clone = 'O(n)', Copy = 'O(n)' },
  strings = {
    Split = 'O(n)', Join = 'O(n)', Contains = 'O(n)',
    HasPrefix = 'O(n)', HasSuffix = 'O(n)', Index = 'O(n)',
    ToLower = 'O(n)', ToUpper = 'O(n)', Trim = 'O(n)',
    Replace = 'O(n)', Count = 'O(n)',
  },
  bytes = {
    Equal = 'O(n)', Compare = 'O(n)', Split = 'O(n)',
    Join = 'O(n)', NewBuffer = 'O(1)', NewBufferString = 'O(n)',
  },
  strconv = {
    Atoi = 'O(n)', Itoa = 'O(n)',
    ParseInt = 'O(n)', FormatInt = 'O(n)',
    ParseFloat = 'O(n)', FormatFloat = 'O(n)',
  },
  ...
  fmt = {
    Print = 'O(n)', Println = 'O(n)', Printf = 'O(n)',
    Sprint = 'O(n)', Sprintf = 'O(n)', Fprintf = 'O(n)',
    Errorf = 'O(n)', Scan = 'O(n)', Fscan = 'O(n)', Sscan = 'O(n)', Sscanf = 'O(n)',
  },
  os = {
    Open = 'O(1)', Create = 'O(1)', Stat = 'O(1)', Lstat = 'O(1)',
    ReadFile = 'O(n)', WriteFile = 'O(n)', Read = 'O(n)', Write = 'O(n)', ReadDir = 'O(n)',
  },
  filepath = { 
    Walk = 'O(n)', WalkDir = 'O(n)', Match = 'O(n)',
    Abs = 'O(n)', Base = 'O(n)', Clean = 'O(n)', Ext = 'O(n)', Join = 'O(n)'
  },
  context  = {
    Background = 'O(1)', TODO = 'O(1)',
    WithTimeout = 'O(1)', WithCancel = 'O(1)', WithDeadline = 'O(1)',
  },
  time     = { Now = 'O(1)', Sleep = 'O(1)', Since = 'O(1)', Until = 'O(1)' },
  math     = {
    Abs = 'O(1)', Max = 'O(1)', Min = 'O(1)',
    Ceil = 'O(1)', Floor = 'O(1)', Round = 'O(1)',
    Pow = 'O(1)', Sqrt = 'O(1)', Log = 'O(1)', Exp = 'O(1)',
    Sin = 'O(1)', Cos = 'O(1)', Tan = 'O(1)', Atan2 = 'O(1)',
  },
  ...
  utf8     = { 
    DecodeRune = 'O(1)', DecodeRuneInString = 'O(1)', 
    RuneCount = 'O(n)', RuneCountInString = 'O(n)', 
    Valid = 'O(n)', ValidString = 'O(n)', ValidRune = 'O(1)'
  },
  reflect  = { DeepEqual = 'O(n)', TypeOf = 'O(1)', ValueOf = 'O(1)' },
  rand     = { Shuffle = 'O(n)', Perm = 'O(n)', Intn = 'O(1)', Float64 = 'O(1)' },
  sync     = { 
    Once = 'O(1)', WaitGroup = 'O(1)', Mutex = 'O(1)', RWMutex = 'O(1)',
    Pool = 'O(1)', Cond = 'O(1)', Map = 'O(1)' -- Map methods are in METHOD_COMPLEXITIES
  },
...
local METHOD_COMPLEXITIES = {
  Write = 'O(n)', Read = 'O(n)', ReadFull = 'O(n)', Sum = 'O(n)', Sum256 = 'O(n)',
  Wait = 'O(n)', Add = 'O(n)', Mul = 'O(n)', Div = 'O(n)', Sub = 'O(n)',
  -- sync.Map
  Range = 'O(n)', Load = 'O(1)', Store = 'O(1)', Delete = 'O(1)',
  -- container/ring
  Next = 'O(1)', Prev = 'O(1)', Move = 'O(n)',
  -- url.Values
  Encode = 'O(n)',
}
...
local function classify_for(for_node)
  ...
  for i = 0, for_node:named_child_count() - 1 do
    ...
    local t  = ch:type()
    if     t == 'for_clause'   then return 'traditional', ch
    elseif t == 'range_clause' then return 'range',       ch
    elseif t == 'block'        then return 'infinite',    nil  -- `for {}`
    else                            return 'while',       ch   -- `for cond {}`
    end
  end
  return 'infinite', nil
end
...
local function update_is_log(node, lines, var_name)
  ...
  if raw:match(p_suffix .. '%*=') or raw:match(p_suffix .. '/=')   then return true end
  if raw:match(p_suffix .. '<<=') or raw:match(p_suffix .. '>>=')  then return true end
  if var_name then
    local esc_var = vim.pesc(var_name)
    if raw:match(esc_var .. '%s*=%s*' .. esc_var .. '%s*[%*%/]') then return true end
    if raw:match(esc_var .. '%s*&=%s*%(.*-.*1%)') then return true end
    if raw:match(esc_var .. '%s*%+=%s*' .. esc_var) then return true end
    if raw:match(esc_var .. '%s*=%s*' .. esc_var .. '%s*%+%s*' .. esc_var) then return true end
  else
    -- Generic patterns for while loops
    if raw:match('&=%s*%(.*-.*1%)') then return true end
    local lhs = raw:match('^%s*([%w_]+)%s*%+=') or raw:match('^%s*([%w_]+)%s*=')
    if lhs then
      local pattern = lhs .. '%s*[%+*]%s*' .. lhs
      if raw:match(pattern) or raw:match('%+=%s*' .. lhs) then return true end
    end
  end
  return false
end
...
local function cond_is_sqrt(cond_node, lines)
  if not cond_node then return false end
  local function walk(n)
    if n:type() == 'binary_expression' then
      -- Operator is anonymous child at index 1 in tree-sitter ordering.
      local op = n:child(1)
      if op and node_text(op, lines) == '*' then return true end
    end
    for i = 0, n:child_count() - 1 do
      if walk(n:child(i)) then return true end
    end
    return false
  end
...
local function cond_is_constant_bound(cond_node, lines)
  ...
  return raw:match('[<>]=?%s*[2-9]%s*$') ~= nil
      or raw:match('[<>]=?%s*%d%d+%s*$') ~= nil
end
...
local function base_complexity_of_for(for_node, lines)
  ...
  if kind == 'infinite' then return 'O(n)' end
  if kind == 'range'    then return 'O(n)' end
  ...
  if kind == 'traditional' then
    ...
    if cond_is_constant_bound(cond, lines) then return 'O(1)'    end
    if cond_is_sqrt(cond, lines)           then return 'O(√n)'   end
    if update_is_log(update, lines, var_name) then return 'O(log n)' end
    
    -- If update is missing from clause, check body (while-style)
    local body = field1(for_node, 'body')
    if body and update_is_log(body, lines, var_name) then return 'O(log n)' end
  ...
  if kind == 'while' then
    ...
    if cond_is_sqrt(clause, lines) then return 'O(√n)' end
    if update_is_log(for_node, lines, var_name) then return 'O(log n)' end
    return 'O(n)'
  end
...
local function complexity_of_make(call_node, lines, mode)
  ...
  if not args_node then return 'O(n)' end
  ...
  if mode == 'time' then
    ...
    if len_raw == '0' then return 'O(1)' end
    -- Constant lengths are O(1)
    if len_raw and len_raw:match('^%d+$') then return 'O(1)' end
    return 'O(n)'
  else
    -- Space: any non-constant length or capacity is O(n)
    local dominant = 'O(1)'
    if len_raw and not len_raw:match('^%d+$') then dominant = 'O(n)' end
    if cap_raw and not cap_raw:match('^%d+$') then dominant = 'O(n)' end
    -- If cap is missing but len is variable, it's still O(n)
    if not cap_raw and len_raw and not len_raw:match('^%d+$') then dominant = 'O(n)' end
    return dominant
  end
end
...
local function collect_ids(body, lines)
  ...
  local function walk(n)
    ...
    for i = 0, n:child_count() - 1 do walk(n:child(i)) end
  end
  walk(body)
  return ids
end
...
local function detect_algorithm(body, func_name, lines)
  ...
  local has = function(...)
    for _, name in ipairs({ ... }) do
      if ids[name] then return true end
    end
    return false
  end
  ...
  local has_sentinel  = raw:match('1e9') or raw:match('math%.Inf')
                     or raw:match('math%.MaxInt') or raw:match('MAX_INT')
  ...
  local has_partition  = raw:match('partition')
                      or (has('left') and has('right') and raw:match('swap'))
  ...
  if (has_pivot or has_partition) and recurse then
    ...
    if raw:match('append') or raw:match('make') then qs_space = 'O(n)' end
  ...
  if (has_parent and has_find and has_union)
  or (has_parent and has_dsu_pattern) then
    return 'O(α(n))', 'O(1)'
  end
  ...
  local has_seg_name = func_name:match('[Ss]egment') or func_name:match('build')
                    or func_name:match('query')        or func_name:match('update')
  if (has_seg_size and has_seg_node)
  or (has_seg_name and (has_seg_size or has_seg_node)) then
...
function M.analyze(bufnr)
      ...
      if fn_child then
        ...
        elseif fn_child:type() == 'identifier' then
          ...
          if name == 'make' then
            ...
            if fe and not fe.is_algorithm then fe.space_complexity = get_dominant(fe.space_complexity, sc) end
  ...
  local by_line = function(a, b) return a.line < b.line end
